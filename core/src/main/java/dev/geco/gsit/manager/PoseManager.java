...
public class PoseManager {
    ...
    public boolean removePose(Player Player, GetUpReason Reason, boolean Safe) {
        ...
        Location returnLocation = (GPM.getCManager().GET_UP_RETURN ? poseSeat.getSeat().getReturnLocation() : poseSeat.getSeat().getLocation().add(0d, GPM.getSitManager().BASE_OFFSET + (Tag.STAIRS.isTagged(poseSeat.getSeat().getBlock().getType()) ? EnvironmentUtil.STAIR_Y_OFFSET : 0d) - GPM.getCManager().S_SITMATERIALS.getOrDefault(poseSeat.getSeat().getBlock().getType(), 0d), 0d));

        if(!GPM.getCManager().GET_UP_RETURN) {

            Location playerLocation = Player.getLocation();

            returnLocation.setYaw(playerLocation.getYaw());
            returnLocation.setPitch(playerLocation.getPitch());
        }
